<#
.SYNOPSIS
    Assigns the Contributor role on /providers/Microsoft.aadiam to a user.

.DESCRIPTION
    Grants the Azure RBAC Contributor role on the /providers/Microsoft.aadiam scope
    to a specified user account. This permission is required to configure diagnostic
    settings (e.g. audit logs, sign-in logs) on an Entra ID tenant.
...
#>
Write-Host "=== Entra Diagnostic Settings - Role Assignment ===" -ForegroundColor Cyan
Write-Host "This assigns the Contributor role on /providers/Microsoft.aadiam"
...
# Assign role
Write-Host "Assigning Contributor role on /providers/Microsoft.aadiam..." -ForegroundColor Cyan
try {
